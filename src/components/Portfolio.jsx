...
export default function Portfolio() {
  ...
  return (
    <section id="portfolio" className="py-24 bg-blue-900" ref={sectionRef}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-full text-sm font-semibold transition-all duration-200 ${
                activeFilter === f
                  ...
                  : "bg-blue-800 text-slate-400 hover:bg-blue-800 hover:text-white border border-slate-700"
              }`}
            >
              {f}
            </button>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((project, i) => (
            <article
              ...
              className={`group relative bg-blue-800 border border-slate-700 rounded-2xl overflow-hidden cursor-pointer
            ...
            >
              ...
              <div className="relative h-48 overflow-hidden">
                ...
                {/* Year */}
                <div className="absolute top-3 right-3 bg-blue-900/80 backdrop-blur-sm text-slate-300 text-xs font-semibold px-2.5 py-1 rounded-lg">
                  {project.year}
                </div>

                {/* Hover overlay */}
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-blue-900/40 backdrop-blur-sm">
        ...
        <div className={`text-center mt-10 transition-all duration-700 delay-400 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
          <button className="inline-flex items-center gap-2 border border-slate-700 hover:border-blue-500 text-slate-300 hover:text-white font-semibold px-8 py-3.5 rounded-xl transition-all duration-200 hover:bg-blue-800">
            Xem tất cả dự án
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {selected && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-blue-900/90 backdrop-blur-sm"
          onClick={() => setSelected(null)}
        >
          <div
            className="bg-blue-800 border border-slate-700 rounded-2xl max-w-2xl w-full shadow-2xl overflow-hidden"
          ...
          >
            <div className="relative">
              ...
              <button
                onClick={() => setSelected(null)}
                className="absolute top-4 right-4 bg-blue-900/80 hover:bg-blue-900 text-white w-9 h-9 rounded-full flex items-center justify-center transition-colors"
              >
                ✕
              </button>
            ...
            <div className="p-6 -mt-4 relative z-10">
              ...
              <div className="flex justify-end mt-6">
                <button
                  onClick={() => setSelected(null)}
                  className="bg-blue-800 hover:bg-slate-600 text-white text-sm font-semibold px-6 py-2.5 rounded-xl transition-colors"
                >
                  Đóng
                </button>
              </div>
